#!/bin/bash
# Sand Garden Firmware Build and Upload Script
# Supports both OTA and USB upload modes

set -e  # Exit on error

# Default to OTA upload
UPLOAD_MODE="ota"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --usb)
      ...
      ;;
    -h|--help)
      echo "Usage: $0 [--ota|--usb]"
      echo ""
      echo "Options:"
      echo "  --ota    Upload via OTA to sand-garden.local (default)"
      echo "  --usb    Upload via USB"
      echo ""
      echo "Environment variables:"
      echo "  CLEAR_CACHE=1    Clear entire build cache before compiling"
      exit 0
      ;;
    *)
...
# Find espota.py dynamically to handle version changes (only needed for OTA)
if [ "$UPLOAD_MODE" = "ota" ]; then
...
if [ "$UPLOAD_MODE" = "ota" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "  4. Use USB upload as fallback: $0 --usb"
    exit 1
  fi
else
  # USB Upload
  echo "Step 5: Uploading via USB..."
  ...
  if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "=========================================="
    echo "✗ USB Upload failed!"
    echo "=========================================="
    echo ""
    echo "Troubleshooting:"
    echo "  1. Check if device is connected via USB"
    echo "  2. Verify the correct port is detected"
    echo "  3. Try: $ARDUINO_CLI board list"
    exit 1
  fi
fi
